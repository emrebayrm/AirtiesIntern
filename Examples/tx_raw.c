...
static unsigned int trans(unsigned char c);
static void usage();
/*****************************************************************************
...
int main(int argc, char *argv[])
{
	...
	arg_ifname = argv[1];
	arg_data = argv[2];
	arg_ether = ETHER_TYPE;
	if (argc == 4) {
		arg_ether = hex_to_uint16(argv[3]);
	}
	fprintf(stderr, "ether type 0x%x\n", arg_ether);
	...
	sfd = net_create_raw_socket(ifname, arg_ether, 0);
	if (sfd == -1) {
	...
	hdr->eth.ether_type = htons(arg_ether);
